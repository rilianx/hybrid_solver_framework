...
def variables(inst: CVRPInstance) -> dict[str, tuple[float, float, str]]:
    ...
    for c in inst.customers:
        vars_[_u_name(c)] = (0.0, float(inst.capacity), "continuous")
...
def from_assignment(inst: CVRPInstance, x) -> "sol":
    ...
    while start is not None and start != 0:
        route = []
        cur = start
        while cur != 0 and cur is not None and cur not in seen_customers:
            route.append(cur)
        ...
        for j in range(1, n + 1):
            if float(x.get(_arc_name(0, j), 0.0)) > 0.5 and j not in seen_customers:
                start = j
                break
...
def constraint_families(inst: CVRPInstance) -> dict[str, list[tuple[dict[str, float], str, float]]]:
    ...
    for c in inst.customers:
        ...
        fam["capacidad"].append(({_u_name(c): 1.0}, "<=", cap))
    ...
    for i in inst.customers:
        for j in inst.customers:
            if i == j:
                continue
            # u_j - u_i + M x_ij >= d_j
            coeffs = {_u_name(j): 1.0, _u_name(i): -1.0, _arc_name(i, j): big_m}
            fam["capacidad"].append((coeffs, ">=", float(inst.demand[j])))

    for j in inst.customers:
        # If route starts at j, then u_j >= d_j
        fam["capacidad"].append(({_u_name(j): 1.0, _arc_name(0, j): big_m}, ">=", float(inst.demand[j])))
