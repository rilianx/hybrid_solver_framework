...
def _solve_aux_values(inst: CLSPInstance, y):
    ...
    short_val = {f"short[{i},{t}]": float(pulp.value(short2[i][t]) or 0.0) for i in range(n_items) for t in range(n_periods)}
    return x_val, inv_val, short_val
...
def aux_values(inst, sol) -> dict[str, float]:
    y = _canonical(sol)
    x_val, inv_val, short_val = _solve_aux_values(inst, y)
    out = {}
    out.update(x_val)
    out.update(inv_val)
    out.update(short_val)
    return out
...
def constraint_families(inst) -> dict[str, list[tuple[dict[str, float], str, float]]]:
    ...
    for i in range(n_items):
        for t in range(n_periods):
            coeffs = {f"inv[{i},{t}]": 1.0, f"x[{i},{t}]": 1.0, f"short[{i},{t}]": 1.0}
            if t > 0:
