...
def _plan_eval_lp(inst: CLSPInstance, sol):
    ...
    prob.solve(__import__("pulp").PULP_CBC_CMD(msg=False))
    status = __import__("pulp").LpStatus[prob.status]
    ...
    xval = {(i, t): float(__import__("pulp").value(x[i, t])) for i in range(n_items) for t in range(n_periods)}
    invval = {(i, t): float(__import__("pulp").value(inv[i, t])) for i in range(n_items) for t in range(n_periods)}
    shortval = {(i, t): float(__import__("pulp").value(short[i, t])) for i in range(n_items) for t in range(n_periods)}
...
def constraint_families(inst) -> dict[str, list[tuple[dict[str, float], str, float]]]:
    ...
    for i in range(n_items):
        for t in range(n_periods):
            coeffs = {f"inv_{i}_{t}": 1.0, f"x_{i}_{t}": -1.0}
            if t > 0:
...
def variable_groups(inst) -> dict[str, list[str]]:
    ...
    for name in ys:
        parts = name.split("_")
        t = int(parts[-1])
        groups[f"g{min(3, (4 * t) // max(1, n_periods))}"].append(name)

    for idx, name in enumerate(ys):
        if all(name not in g for g in groups.values()):
