...
def _solve_lp(inst: CLSPInstance, sol):
    sol = canonical(sol)
    n_items = inst.n_items
    n_periods = inst.n_periods
    ...
    values = {
        "status": status,
        "x": {(i, t): float(pulp.value(x[i, t])) for i in range(n_items) for t in range(n_periods)},
        "inv": {(i, t): float(pulp.value(inv[i, t])) for i in range(n_items) for t in range(n_periods)},
        "short": {(i, t): float(pulp.value(short[i, t])) for i in range(n_items) for t in range(n_periods)},
    }
    return values
...
@lru_cache(maxsize=None)
def _solve_plan(inst: CLSPInstance, sol):
    data = _solve_lp(inst, sol)
    if data["status"] != "Optimal":
        return {"status": data["status"], "shortage": float("inf"), "inventory": float("inf")}
    shortage = sum(data["short"].values())
    inventory = sum(
        inst.holding_cost[i] * data["inv"][i, t]
        for i in range(inst.n_items)
        for t in range(inst.n_periods)
    )
    return {"status": data["status"], "shortage": float(shortage), "inventory": float(inventory)}
...
def variables(inst: CLSPInstance) -> dict[str, tuple[float, float, str]]:
    ...
    for i in range(n_items):
        for t in range(n_periods):
            ...
            vars_[f"inv_{i}_{t}"] = (0.0, max_demand, "continuous")
            vars_[f"short_{i}_{t}"] = (0.0, max_demand, "continuous")
    return vars_
...
def constraint_families(inst: CLSPInstance) -> dict[str, list[tuple[dict[str, float], str, float]]]:
    ...
    for i in range(n_items):
        for t in range(n_periods):
            families["setup_link"].append(({f"x_{i}_{t}": 1.0, f"y_{i}_{t}": -float(inst.capacity[t])}, "<=", 0.0))

            coeffs_dem = {f"short_{i}_{t}": 1.0}
            families["demanda"].append((coeffs_dem, "==", 0.0))

            coeffs_bal: dict[str, float] = {
                f"inv_{i}_{t}": 1.0,
                f"x_{i}_{t}": -1.0,
                f"short_{i}_{t}": -1.0,
            }
            ...
            families["setup_link"].append((coeffs_bal, "==", -float(inst.demand[i][t])))
