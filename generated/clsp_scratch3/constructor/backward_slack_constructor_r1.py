...
class BackwardSlackConstructor:
    ...
    def build(self, inst, rng: Random):
        ...
        for i in item_order:
            ...
            active.sort()
            # ensure at least one setup exists
            if not active:
        ...
        for _ in range(inst.n_items * inst.n_periods * 4):
            if problem.is_feasible(sol):
                break
            slack = [(inst.capacity[t] - sum(inst.setup_time[i] for i in range(inst.n_items) if sol[i][t]), t) for t in range(inst.n_periods)]
        ...
        if not problem.is_feasible(sol):
            ...
            for i in range(inst.n_items):
                if any(sol[i]):
                    continue
                best_t = max(range(inst.n_periods), key=lambda t: (inst.capacity[t], -t, rng.random()))
