...
def _evaluate_plan(inst: CLSPInstance, sol: tuple[tuple[bool, ...], ...]) -> tuple[float, float, float]:
    ...
    # Network:
    # source -> period nodes (capacity left after setups)
    # period nodes -> production nodes (i,t) if item i has setup in t
    # production nodes -> demand nodes (i,u), u >= t, with inventory cost
    # demand nodes -> sink with demand quantities
    src = 0
    period0 = 1
...
def cost_terms(inst, sol) -> dict[str, float]:
    ...
    terms["total"] = float(setup_term + inv_cost)
    return terms
