...
COMPONENT = {
    "name": "clsp_mip_view",
    "slot": "solution_view",
    "compatible_skeletons": ["mip", "matheuristic"],
    "requires": [],
    "params": {},
}


def _mp_get_attr(inst, *names):
    for name in names:
        if hasattr(inst, name):
            return getattr(inst, name)
    raise AttributeError(f"instance missing any of attributes {names}")
...
def _mp_n_items(inst) -> int:
    return int(_mp_get_attr(inst, "n_items"))


def _mp_n_periods(inst) -> int:
    return int(_mp_get_attr(inst, "n_periods"))


def _mp_demand(inst):
    return _mp_get_attr(inst, "demand")


def _mp_setup_cost(inst):
    return _mp_get_attr(inst, "setup_cost", "s")


def _mp_holding_cost(inst):
    return _mp_get_attr(inst, "holding_cost", "h")


def _mp_setup_time(inst):
    return _mp_get_attr(inst, "setup_time", "st")


def _mp_capacity(inst):
    return _mp_get_attr(inst, "capacity", "cap")


def _mp_canonical(sol):
    if sol is None:
        return None
    return tuple(tuple(bool(v) for v in row) for row in sol)


def _mp_big_m(inst) -> float:
    demand = _mp_demand(inst)
    capacity = _mp_capacity(inst)
    m = 1.0
    for i in range(_mp_n_items(inst)):
        m = max(m, sum(float(d) for d in demand[i]))
    for t in range(_mp_n_periods(inst)):
        m = max(m, float(capacity[t]))
    return m


def variables(inst) -> dict[str, tuple[float, float, str]]:
    n_items = _mp_n_items(inst)
    n_periods = _mp_n_periods(inst)
...
class _MP_MinCostFlow:
    def __init__(self, n: int):
        self.g = [[] for _ in range(n)]

    def add_edge(self, u: int, v: int, cap: float, cost: float) -> None:
        self.g[u].append(_MP_Edge(v, len(self.g[v]), cap, cost))
        self.g[v].append(_MP_Edge(u, len(self.g[u]) - 1, 0.0, -cost))
...
def _mp_evaluate_plan(inst: CLSPInstance, sol: tuple[tuple[bool, ...], ...]) -> tuple[float, float, float, list[list[float]], list[list[float]]]:
    n_items = _mp_n_items(inst)
    n_periods = _mp_n_periods(inst)
    demand = _mp_demand(inst)
    setup_cost = _mp_setup_cost(inst)
    holding_cost = _mp_holding_cost(inst)
    setup_time = _mp_setup_time(inst)
    capacity = _mp_capacity(inst)
    ...
    if any(c < -1e-12 for c in remaining_capacity):
        return total_demand, setup_term, 0.0, [[0.0] * n_periods for _ in range(n_items)], [[0.0] * n_periods for _ in range(n_items)]
    ...
    return unmet, setup_term, inv_cost, q, inv


def to_assignment(inst, sol) -> dict[str, float]:
    sol = _mp_canonical(sol)
    n_items = _mp_n_items(inst)
    n_periods = _mp_n_periods(inst)
...
def aux_values(inst, sol) -> dict[str, float]:
    ...
    n_periods = _mp_n_periods(inst)
    sol = _mp_canonical(sol)
    unmet, _, _, q, inv = _mp_evaluate_plan(inst, sol)
    aux: dict[str, float] = {}
    for i in range(n_items):
        for t in range(n_periods):
...
def variable_groups(inst) -> dict[str, list[str]]:
    n_items = _mp_n_items(inst)
    n_periods = _mp_n_periods(inst)
    groups: dict[str, list[str]] = {"g0": [], "g1": [], "g2": [], "g3": []}
    for t in range(n_periods):
        for i in range(n_items):
            groups[f"g{t % 4}"].append(f"y_{i}_{t}")
    return groups


class CLSPMIPView:
    def __init__(self, problem: CLSPInstance):
        self.problem = problem
