...
def variables(inst) -> dict[str, tuple[float, float, str]]:
    ...
    demand = _demand(inst)
    for i in range(n_items):
        big = max(big, sum(float(d) for d in demand[i]))
    for t in range(n_periods):
        big = max(big, float(_capacity(inst)[t]))
    big = max(big, 1.0)
...
def _solve_plan(inst, sol):
    ...
    total_demand = sum(float(demand[i][t]) for i in range(n_items) for t in range(n_periods))
    unmet = max(0.0, total_demand - flow)
    setup_term = sum(float(setup_cost[i]) for i in range(n_items) for t in range(n_periods) if sol[i][t])
    return {
        "unmet": unmet,
        "setup": setup_term,
        "inventario": inv_cost,
...
def constraint_families(inst) -> dict[str, list[tuple[dict[str, float], str, float]]]:
    ...
    for i in range(n_items):
        ...
        rhs = 0.0
        for t in range(n_periods):
            coeff[f"q_{i}_{t}"] = coeff.get(f"q_{i}_{t}", 0.0) + 1.0
            rhs += float(demand[i][t])
            fams["demanda"].append(
                (
                    {f"q_{i}_{u}": 1.0 for u in range(t + 1)},
                    ">=",
                    float(sum(demand[i][u] for u in range(t + 1))),
                )
            )
...
def objective_terms(inst) -> dict[str, tuple[dict[str, float], float]]:
    ...
    terms["setup"] = (coeff_setup, 0.0)
    terms["inventario"] = (coeff_inv, 0.0)
    return terms
