...
def variables(inst) -> dict[str, tuple[float, float, str]]:
    ...
    # Variable auxiliar opcional para penalización; queda libre salvo por su cota.
    vars_[_pen_name()] = (0.0, 1.0, "continuous")
    return vars_
...
def aux_values(inst, sol) -> dict[str, float]:
    ...
    vals: dict[str, float] = {_pen_name(): 0.0}
    ...
    for c in inst.customers:
        vals.setdefault(_u_name(c), float(inst.demand[c]))

    # Si hubiera violación en la solución dada, activamos un marcador finito.
    v = violations(inst, sol)
    vals[_pen_name()] = 1.0 if (v["visita"] > 0.0 or v["capacidad"] > 0.0) else 0.0
    return vals
...
def constraint_families(inst) -> dict[str, list[tuple[dict[str, float], str, float]]]:
    ...
    for c in inst.customers:
        ...
        fams["capacidad.cota_sup"].append(({_u_name(c): 1.0}, "<=", float(inst.capacity)))
    ...
    for i in inst.customers:
        for j in inst.customers:
            if i == j:
                continue
            coef = {_u_name(j): 1.0, _u_name(i): -1.0, _x_name(i, j): q}
...
def objective_terms(inst) -> dict[str, tuple[dict[str, float], float]]:
    ...
    coef[_pen_name()] = feasible_upper + 1.0
...
def variable_groups(inst) -> dict[str, list[str]]:
    n = inst.n_customers
    structs = structural_variables(inst)
    if not structs:
        return {}
    ...
    for i in range(n + 1):
        for j in range(n + 1):
            if i == j:
                continue
            if i == 0:
                gid = (j - 1) % 4 if j >= 1 else 0
            else:
                gid = (i - 1) % 4
            groups[f"g{gid}"].append(_x_name(i, j))

    # Rebalance to ensure no group dominates too much on small instances.
    # Keep four non-empty groups whenever possible.
