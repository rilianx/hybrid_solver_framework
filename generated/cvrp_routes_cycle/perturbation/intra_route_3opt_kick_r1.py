...
class IntraRoute3OptKickPerturbation:
    ...
    def perturb(self, sol, strength: float, rng: Random):
        ...
        if not nontrivial:
            # fallback: reverse a route with at least 2 customers, or swap first two routes
        ...
        # A compact 3-opt style kick: pick three cut points and reconnect pieces.
        i, j, k = sorted(rng.sample(range(1, n), 2))
        # Ensure three segments exist; if not, use a reverse as fallback.
        if i == j or j >= n:
            r.reverse()
        else:
            mode = int(strength) % 3
