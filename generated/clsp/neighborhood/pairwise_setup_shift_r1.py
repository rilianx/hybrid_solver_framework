...
class PairwiseSetupShiftNeighborhood:
    """Mueve un setup de un período a otro para el mismo ítem: (i, t_from, t_to)."""
    ...
    def apply(self, sol, m):
        ...
        return tuple(
            tuple(
                (False if (ii == i and tt == t_from) else True if (ii == i and tt == t_to) else bit)
    ...
    def undo(self, sol, m):
        ...
        return tuple(
            tuple(
                (True if (ii == i and tt == t_from) else False if (ii == i and tt == t_to) else bit)
                for tt, bit in enumerate(row)
            )
            for ii, row in enumerate(sol)
        )
