...
def _mip_n_items(inst) -> int:
    return int(_mip_get_attr(inst, "n_items"))


def _mip_n_periods(inst) -> int:
    return int(_mip_get_attr(inst, "n_periods"))


def _mip_demand(inst):
    return _mip_get_attr(inst, "demand")


def _mip_setup_cost(inst):
    return _mip_get_attr(inst, "setup_cost", "s")


def _mip_holding_cost(inst):
    return _mip_get_attr(inst, "holding_cost", "h")


def _mip_setup_time(inst):
    return _mip_get_attr(inst, "setup_time", "st")


def _mip_capacity(inst):
    return _mip_get_attr(inst, "capacity", "cap")


def _mip_canonical(sol):
    if sol is None:
        return None
    return tuple(tuple(bool(v) for v in row) for row in sol)


def _mip_big_m(inst) -> float:
    demand = _mip_demand(inst)
    capacity = _mip_capacity(inst)
    m = 1.0
    for i in range(_mip_n_items(inst)):
        m = max(m, sum(float(d) for d in demand[i]))
    for t in range(_mip_n_periods(inst)):
        m = max(m, float(capacity[t]))
    return m


def variables(inst) -> dict[str, tuple[float, float, str]]:
    n_items = _mip_n_items(inst)
    n_periods = _mip_n_periods(inst)
...
class _MIP_MinCostFlow:
    def __init__(self, n: int):
        self.g = [[] for _ in range(n)]

    def add_edge(self, u: int, v: int, cap: float, cost: float) -> None:
        self.g[u].append(_MIP_Edge(v, len(self.g[v]), cap, cost))
        self.g[v].append(_MIP_Edge(u, len(self.g[u]) - 1, 0.0, -cost))
...
def _mip_evaluate_plan(inst: CLSPInstance, sol: tuple[tuple[bool, ...], ...]) -> tuple[float, float, float, list[list[float]], list[list[float]]]:
    n_items = _mip_n_items(inst)
    n_periods = _mip_n_periods(inst)
    demand = _mip_demand(inst)
    setup_cost = _mip_setup_cost(inst)
    holding_cost = _mip_holding_cost(inst)
    setup_time = _mip_setup_time(inst)
    capacity = _mip_capacity(inst)
...
def aux_values(inst, sol) -> dict[str, float]:
    ...
    n_periods = _mip_n_periods(inst)
    sol = _mip_canonical(sol)
    _, _, _, q, inv = _mip_evaluate_plan(inst, sol)
    aux: dict[str, float] = {}
    for i in range(n_items):
        for t in range(n_periods):
