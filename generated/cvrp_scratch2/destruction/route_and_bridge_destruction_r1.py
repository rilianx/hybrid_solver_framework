...
class RouteAndBridgeDestruction:
    """Libera una ruta completa, o bien varios clientes de una ruta corta, incluyendo arcos puente."""
    ...
    def destroy(self, sol, ratio: float, rng: Random) -> tuple[Any, set[str]]:
        ...
        route_len = len(route)
        target = max(1, min(route_len, int(round(ratio * self.inst.n_customers))))

        if target >= route_len or rng.random() < 0.5:
            block = set(route)
        else:
            start = rng.randrange(route_len)
            block = {route[(start + t) % route_len] for t in range(min(target, route_len))}
