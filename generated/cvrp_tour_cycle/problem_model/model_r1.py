...
def variables(inst) -> dict[str, tuple[float, float, str]]:
    ...
    # Arc variables on the augmented graph {0..n}.
    for i in range(n + 1):
        for j in range(n + 1):
            if i != j:
                vars_[f"x_{i}_{j}"] = (0.0, 1.0, "binary")

    # Load variables for customers.
    for i in inst.customers:
        vars_[f"u_{i}"] = (float(inst.demand[i]), float(inst.capacity), "continuous")
...
def to_assignment(inst, sol) -> dict[str, float]:
    ...
    # Ensure all customer loads are present even if isolated.
    for c in inst.customers:
        u.setdefault(f"u_{c}", float(inst.demand[c]))
...
def aux_values(inst, sol) -> dict[str, float]:
    ...
    vals: dict[str, float] = {}
    for c in inst.customers:
        vals[f"u_{c}"] = float(inst.demand[c])

    # Customer loads along the decoded routes.
    for route in routes:
        load = 0.0
        for c in route:
            load += float(inst.demand[c])
...
def from_assignment(inst, x) -> "sol":
    # Reconstruct routes from the successor relation, then concatenate them.
    succ = {}
    for i in range(inst.n_customers + 1):
        for j in range(inst.n_customers + 1):
            if i != j and x.get(f"x_{i}_{j}", 0.0) > 0.5:
                succ[i] = j

    routes: list[list[int]] = []
    used = set()
    for start in range(1, inst.n_customers + 1):
        if x.get(f"x_0_{start}", 0.0) > 0.5 and start not in used:
            route = []
            cur = start
    ...
    # Any remaining customers are appended deterministically.
    remaining = [c for c in inst.customers if c not in used]
    for c in remaining:
        routes.append([c])
...
def constraint_families(inst) -> dict[str, list[tuple[dict[str, float], str, float]]]:
    ...
    # Depot balance: same number of departures and returns.
    fam["visita"].append(({f"x_0_{j}": 1.0 for j in inst.customers}, "==", {f"x_{i}_0": 1.0 for i in inst.customers}))  # type: ignore

    # Capacity / load propagation (MTZ-like).
    Q = float(inst.capacity)
    for i in inst.customers:
        fam["capacidad"].append(({f"u_{i}": 1.0}, ">=", float(inst.demand[i])))
        fam["capacidad"].append(({f"u_{i}": 1.0}, "<=", Q))
        for j in inst.customers:
            if i != j:
                fam["capacidad"].append(
                    ({f"u_{j}": 1.0, f"u_{i}": -1.0, f"x_{i}_{j}": -Q}, ">=", float(inst.demand[j]) - Q)
                )
        fam["capacidad"].append(({f"u_{i}": 1.0, f"x_0_{i}": -Q}, ">=", float(inst.demand[i]) - Q))

    return fam


def objective_terms(inst) -> dict[str, tuple[dict[str, float], float]]:
    terms: dict[str, tuple[dict[str, float], float]] = {}
    n = inst.n_customers
...
def variable_groups(inst) -> dict[str, list[str]]:
    ...
    customers = list(inst.customers)
    mid = max(1, len(customers) // 2)
    first = set(customers[:mid])
    second = set(customers[mid:])
    ...
    for i in range(n + 1):
        for j in range(n + 1):
            ...
            name = f"x_{i}_{j}"
            if i == 0 or j == 0:
                groups["depot"].append(name)
            elif i in first or j in first:
                groups["first_half"].append(name)
            else:
                groups["second_half"].append(name)

    groups["load"] = [f"u_{i}" for i in inst.customers]
    return groups
