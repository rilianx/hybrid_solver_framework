...
def constraint_families(inst) -> dict[str, list[tuple[dict[str, float], str, float]]]:
    ...
    # Load variables and propagation.
...
def variable_groups(inst) -> dict[str, list[str]]:
    ...
    q1 = max(1, len(customers) // 4)
    q2 = max(q1 + 1, len(customers) // 2)
    q3 = max(q2 + 1, (3 * len(customers)) // 4)

    A = set(customers[:q1])
    B = set(customers[q1:q2])
    C = set(customers[q2:q3])
    D = set(customers[q3:])
