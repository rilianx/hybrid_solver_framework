...
def constraint_families(inst) -> dict[str, list[tuple[dict[str, float], str, float]]]:
    ...
    dep_out = {f"x_0_{j}": 1.0 for j in inst.customers}
    dep_in = {f"x_{i}_0": -1.0 for i in inst.customers}
    dep_out.update(dep_in)
    fam["visita"].append((dep_out, "==", 0.0))
    ...
    for i in inst.customers:
        ...
        for j in inst.customers:
            ...
            fam["capacidad"].append(
                ({f"u_{j}": 1.0, f"u_{i}": -1.0, f"x_{i}_{j}": -Q}, ">=", dj - Q)
            )
...
def variable_groups(inst) -> dict[str, list[str]]:
    n = inst.n_customers
    vars_by_group: dict[str, list[str]] = {"g1": [], "g2": [], "g3": [], "g4": []}

    # Partition by tail node blocks so each group contains a compact set of arcs
    # and interacting arcs are mainly within/near the same structural neighborhood.
    ...
    for b in range(4):
        ...
        cuts.append(set(tails[start : start + size]))
        start += size

    group_names = ["g1", "g2", "g3", "g4"]
    tail_to_group = {}
    for gname, block in zip(group_names, cuts):
        for t in block:
            tail_to_group[t] = gname

    for i in range(n + 1):
        gname = tail_to_group.get(i, "g4")
        for j in range(n + 1):
            if i != j:
                vars_by_group[gname].append(f"x_{i}_{j}")
