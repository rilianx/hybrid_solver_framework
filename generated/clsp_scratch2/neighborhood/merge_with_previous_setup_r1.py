...
class MergeWithPreviousSetupNeighborhood:
    """Apaga setups redundantes de un ítem cuando un setup anterior puede absorber la demanda."""

    def __init__(self, problem: ProblemModel, max_lookback: int = 3, min_savings_margin: float = 0.0):
        ...
        for i in range(inst.n_items):
            for t in range(inst.n_periods):
                if not self._can_merge(sol, i, t):
                    continue
                # Heurística: solo candidatos donde el setup de t parece "pequeño" en utilidad.
...
def build_component(problem, max_lookback: int = 3, min_savings_margin: float = 0.0):
    return MergeWithPreviousSetupNeighborhood(problem, max_lookback=max_lookback, min_savings_margin=min_savings_margin)
